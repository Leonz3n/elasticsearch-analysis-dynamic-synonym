 ...
 *
 * <p>
 * Note that with the current implementation, parsing is greedy, so whenever multiple parses would
 * apply, the rule starting the earliest and parsing the most tokens wins. For example if you have
 * these rules:
 *
 * <pre>
 *   a -> x
 ...
 * </pre>
 * <p>
 * Then input <code>a b c d e</code> parses to <code>y b c d</code>, ie the 2nd rule "wins" because
 * it started earliest and matched the most input tokens of other rules starting at that point.
 * </p>
 *
 * <p>
 * A future improvement to this filter could allow non-greedy parsing, such that the 3rd rule would
 * win, and also separately allow multiple parses, such that all 3 rules would match, perhaps even
 ...
 * tokens. This is not a correct solution, as really the output should be an arbitrary
 * graph/lattice. For example, with the above match, you would expect an exact
 * <code>PhraseQuery</code> <code>"y b c"</code> to match the parsed tokens, but it will fail to do
 * so. This limitation is necessary because Lucene's TokenStream (and index) cannot yet represent an
 * arbitrary graph.
 * </p>
 *
 * <p>
 * <b>NOTE</b>: If multiple incoming tokens arrive on the same position, only
 * the first token at that position is used for parsing. Subsequent tokens simply pass through and
 * are not parsed. A future improvement would be to allow these tokens to also be matched.
 * </p>
 */
...
public final class DynamicSynonymFilter extends TokenFilter {
   ...
   *                   if you set this to true, its your responsibility to lowercase the input
   *                   entries when you create the {@link SynonymMap}
   */
  DynamicSynonymFilter(TokenStream input, SynonymMap synonyms, boolean ignoreCase) {
    super(input);
